...
@testset "Dipolar hard spheres, MSA" begin
    ...
    @test f(0.0) ≈ (0.455625, 0.6291407983765894, 1.2666736611367042)
    @test f(π/2) ≈ (0.5162698774690883, 0.6850767960631556, 1.1938720996192682)
    @test f(1π ) ≈ (0.7166968096587476, 0.8416086006775434, 1.0666596582612127)
    @test f(2π ) ≈ (1.0831625291823719, 1.0426349627796052, 0.9836365170156721)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.2
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (0.20897959183673484, 0.4431390440180322, 1.5229321819837567)
    @test f(π/2) ≈ (0.2492889697353935, 0.5035359373018747, 1.3590195670624214)
    @test f(1π ) ≈ (0.42259178223764265, 0.7056963342919332, 1.1112828002460329)
    @test f(2π ) ≈ (1.2365374806798426, 1.0871649764504834, 0.9728720629632218)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.3
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (0.0937890624999999, 0.33702806816959363, 1.7633203545450626)
    @test f(π/2) ≈ (0.113580452287522, 0.391990692356982, 1.4979049530982467)
    @test f(1π ) ≈ (0.2083407060258776, 0.5976735425934862, 1.142269051998251)
    @test f(2π ) ≈ (1.513704692974072, 1.1316981741146153, 0.9653800070336875)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.4
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (0.04, 0.2700545515466438, 1.9882472781755554)
    @test f(π/2) ≈ (0.04834624242434464, 0.3184635771562895, 1.6157348484418494)
    @test f(1π ) ≈ (0.0889840427994623, 0.5132862854019397, 1.164806497625769)
    @test f(2π ) ≈ (1.7888899102094844, 1.1754992038280996, 0.959870575710502)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.5
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (0.015625000000000007, 0.2244848150807989, 2.19952173374085)
    @test f(π/2) ≈ (0.01870914640113364, 0.26704088038841756, 1.717078039617591)
    @test f(1π ) ≈ (0.0333665751636566, 0.44699097699243284, 1.1818864428745708)
    @test f(2π ) ≈ (1.0433145711209884, 1.218279674681617, 0.9556335505378297)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.6
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (0.005289256198347106, 0.19168706560349014, 2.399024573085883)
    @test f(π/2) ≈ (0.006257471729168288, 0.22934004449014636, 1.805425696890689)
    @test f(1π ) ≈ (0.010689789920403656, 0.3942033193842698, 1.1952745418340207)
    @test f(2π ) ≈ (0.23121862372838764, 1.259913401542428, 0.9522584797097164)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.7
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (0.0014062500000000021, 0.1670469612108733, 2.5883898638161775)
    @test f(π/2) ≈ (0.00164315820030793, 0.20064493447924217, 1.8833761824596493)
    @test f(1π ) ≈ (0.002687286985730639, 0.35151915456843674, 1.2060595083159515)
    @test f(2π ) ≈ (0.03729828280050181, 1.3003308303131165, 0.9494945625210339)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.8
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (0.00023668639053254386, 0.14790394033902815, 2.7689702226837616)
    @test f(π/2) ≈ (0.0002733505741836147, 0.17813835606095949, 1.952872588160253)
    @test f(1π ) ≈ (0.00042959576011198774, 0.31648036971131643, 1.2149430519305693)
    @test f(2π ) ≈ (0.004276959009112015, 1.3394797925053805, 0.9471805606574033)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)

    η = 0.9
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
    @test f(0.0) ≈ (1.2755102040816327e-5, 0.13262741072118495, 2.9418730238949813)
    @test f(π/2) ≈ (1.4577626809289744e-5, 0.1600487295395634, 2.015388817420257)
    @test f(1π ) ≈ (2.2128300716842136e-5, 0.28731317162020314, 1.2223958555431893)
    @test f(2π ) ≈ (0.0001727400947607701, 1.3773111025661495, 0.9452081841208045)
    @test f(k∞ ) ≈ (1.0, 1.0, 1.0)
end
