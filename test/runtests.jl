...
@testset "Hard disks, Rosenfeld FMT" begin
    η = 0.1
    f = StructureFactor(HardDisk(), RosenfeldFMT(η))
...
@testset "Hard spheres, Percus–Yevick" begin
    η = 0.1
    f = StructureFactor(HardSphere(), PercusYevick(η))
    @test f(0.0) ≈ 0.455625
...
@testset "Hard spheres, Verlet–Weis" begin
    η = 0.1
    f = StructureFactor(HardSphere(), VerletWeis(η))
...
@testset "Dipolar hard spheres, MSA" begin
    η = 0.1
    f = StructureFactor(DipolarHardSphere(), MSA(PercusYevick, 1.0, η))
